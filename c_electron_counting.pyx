...
cdef void c_electron_counting(float[:, :] image, float[:, :] result, int height, int width, float baseline,
    ...
    # Integration range going from -3 gamma to 3 gamma
    ...
    for k in range(height):
        ...
        i = 0
        while i < width:
            if index != -1 and (image[k, i] - baseline < threshold or i - index >= peaklength):
                divided_counts = integral/counts_divisor
                result[k, index] = divided_counts #find_most_likely_counts(divided_counts) #round(divided_counts)
...
cdef float count_probability(int counts, float x, float mean):
...
cdef float find_most_likely_counts(float x, float mean):
    if round(x) == 0.0:
        return 0.0
    cdef int counter
    ...
    while counter < factorials_table_length:
        current_probability = count_probability(counter, x, mean)
